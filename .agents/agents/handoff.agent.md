...
- Follow the repository's Clickable Reference Policy from [AGENTS.md](../../../context-engine/AGENTS.md): render entity references as markdown links pointing to manifest files or viewer deep links.
...
- Prefer the dominating peer over `"Claude Sonnet 4.5 (copilot)"`, `"Claude Haiku 4.5 (copilot)"`, and the Gemini Flash models — each is beaten by a laddered model on every priced axis, so none has a cost argument in its favour. They remain dispatchable for a stated reason; what is not acceptable is improvising from a vendor family name. See [model-routing.instructions.md](../instructions/workflow/model-routing.instructions.md).
