...
Clippy documentation can be found in the official Rust Clippy usage guide.
...
| --------- | ----| -----|
| `redundant_clone` | Detects unnecessary `clones`, has performance impact | nursery + perf |
| `needless_borrow` group | Removes redundant `&` borrowing | style |
...
| `large_enum_variant` | Warns if an enum has very large variant which is bad for memory. Suggests `Boxing` it | perf |
| `unnecessary_wraps` | If your function always returns `Some` or `Ok`, you don't need `Option`/`Result` | pedantic |
| `clone_on_copy` | Catches accidental `.clone()` on `Copy` types like `u32` and `bool` | complexity |
| `needless_collect` | Prevents collecting and allocating an iterator, when allocation is not needed | nursery |
