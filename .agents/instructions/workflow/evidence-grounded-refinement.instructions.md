...
- [ticket-refinement.agent.md](https://github.com/mankinskin/context-engine/blob/main/.agents/agents/ticket-refinement.agent.md) — applies it to a ticket; evidence is the ticket store, board, and spec stack.
- [mission-planning.agent.md](https://github.com/mankinskin/context-engine/blob/main/.agents/agents/mission-planning.agent.md) — applies it to a raw ask's mission goal; evidence is whatever the dispatching loop above hands over, most often as the interview target of another consumer's own loop rather than a standalone entry point.
