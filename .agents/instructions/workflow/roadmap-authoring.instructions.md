...
**Commit at logical checkpoints.** Every waypoint states the commit checkpoint that preserves the stable state produced by that session package. The checkpoint belongs after the waypoint's validation/review gate and before dependent waypoints start, so later sessions have a durable commit id, message, and documentation trail instead of relying on a dirty working tree. A checkpoint may explicitly say `Commit checkpoint: none (<reason>)` only when the waypoint is read-only or produces no repository change.
...
- **Literal pipe escaping.** Every literal vertical bar inside a waypoint table cell must be escaped as `\|`, including pipes inside inline code, shell commands, regular-expression alternation, selectors, and environment values. Only Markdown column boundaries remain unescaped. For example, write `` `rg -n "foo\|bar" path` `` and `` `TARGET=v1\|v2` `` inside a table cell. Before completing a roadmap edit, inspect the raw Markdown source and verify that each `Prompt` and `Validate` row still has exactly its structural column delimiters after escaped pipes are ignored.
