...
1. **Resolve source dossiers.** Validate each named folder is a completed dossier — it has `README.md` and `ROADMAP.md`. Treat an incomplete dossier (mid-pipeline, no `ROADMAP.md` yet) as a blocker: finish [refine-ingest.prompt.md](../../../../context-engine/.agents/prompts/refine-ingest.prompt.md) on it first, or exclude it.
